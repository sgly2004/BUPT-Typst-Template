...
#let BUPTBachelorThesis(
  ...
  // 数学公式（Typst 0.13+：show 内用 context + here() 替代 locate；selector() 包装以兼容 0.13 选择器要求）
  show selector(math.equation): it => if it.block {
    context {
  ...
  set par(first-line-indent: (amount: 2em, all: true), leading: 1.2em)  // 段内行间距为1.2倍，all:true 使标题后首段也缩进
  ...
  // 目录（Typst 0.13+：footer 中不再使用 locate，改用 context 取当前页码）
  set page(
    footer: context [
      #align(center)[
        #text(font: FONTSET.at("English"), size: FONTSIZE.XiaoWu)[
          #romanNumMap(counter(page).get().at(0))
        ]
      ]
    ]
  )
  ...
  show outline: it => context {
    ...
    let headingList = query(selector(heading).after(loc))
